...
- No background cost worth naming — measured, not hoped: with live
  ...
  The keeper adds ~25µs to a keystroke's round trip (a 120Hz frame is
  ...
  for under 1% CPU instead of pegging a core. Reattaching after a
  relaunch replays a full 1 MiB scrollback ring in ~2 ms. Reproduce
  every number yourself: `./scripts/bench.py`.
